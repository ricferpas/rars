...
#ifdef RISCV2RARS
void hack(void) {
  // evita que se optimizen las funciones que siempre se llaman con el mismo 1er argumento (realmente no sería necesario ahora que las funciones no son estáticas)
  // también se asegura de que las funciones que no se llaman directamente en el código sean identificadas como funciones por riscv2rars y les ponga una línea en blanco al principio
  imagen_print(NULL);
  imagen_copy(NULL, NULL);
  imagen_dibuja_imagen_rotada(NULL, NULL, 0, 0);
  hack(); // no poner esta llamada la última, o se optimiza como tail-call
  tecla_salir();
  tecla_izquierda();
  tecla_derecha();
  tecla_abajo();
